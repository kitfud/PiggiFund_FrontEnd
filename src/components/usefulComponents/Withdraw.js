...
const Withdraw = ({inRecoveryTime,provider,setWalletBalance,piggiContractIndex,setPiggiContractBalance,walletBalance,contract, contractBalance, account, contractIndex}) => {
...
const donationWei = await contract.amountDonatedInContract(account,contractIndex)
const donationETH = parseFloat(ethers.utils.formatEther(donationWei))
console.log("type donation " +typeof(donationETH))
console.log("donation amount is: "+ donationETH)
...
<Card sx={{marginBottom:5}}>
  {
  !processing ?<>
<Typography variant="h2" sx={{ fontSize: 15, fontWeight: 600, marginBottom:2 }} >Funded: <Typography color="red"> {userdonation} ETH</Typography></Typography>

{
  inRecoveryTime && userdonation>0 ?
